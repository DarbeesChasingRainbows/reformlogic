...
export default function DiagnosticForm() {
  ...
  const challenge = useSignal("");
  const preferred = useSignal("");
    ...
    return (
      <div class="bg-emerald-900/20 border border-emerald-500/30 rounded-lg p-8 text-center">
        ...
        <p class="text-slate-400">
          I'll review your details and reach out within 24 hours to schedule
    ...
    <form onSubmit={handleSubmit} class="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          onInput={(e) =>
            challenge.value = (e.target as HTMLTextAreaElement).value}
          ...
          placeholder="Describe your architecture challenge, current stack, or what decision you need help with..."
        />
      </div>
      <div>
        <label class="block text-xs font-mono text-slate-500 uppercase tracking-widest mb-2">
        ...
        <input
          ...
          onInput={(e) =>
            preferred.value = (e.target as HTMLInputElement).value}
          class="w-full bg-slate-900 border border-slate-800 text-slate-200 px-4 py-3 rounded-lg text-sm focus:outline-none focus:border-indigo-500 transition-colors"
      ...
      >
        {status.value === "sending" ? "Sending..." : "Get Started"}
      </button>
    </form>
  );
}
