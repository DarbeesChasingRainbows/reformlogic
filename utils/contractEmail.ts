/**
 * Email templates for signed contracts.
 * Sends comprehensive signed contract records to both parties.
 */

import { type EmailResult, getNotificationEmail, send } from "./email.ts";
...
/**
 * Send the signed contract record to both the consultant and the client.
 * The email contains the full contract text plus all signature metadata,
 * serving as the legally binding record under the ESIGN Act.
 */
export async function sendSignedContract(
  ...
  const consultantOk = consultantResult.status === "fulfilled" &&
    consultantResult.value.ok;
