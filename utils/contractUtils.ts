/**
 * Contract utility functions — variable interpolation, signature ID generation,
 * query parameter parsing, and HTML escaping for XSS prevention.
 */
...
/**
 * Parse contract-related query parameters from a URL.
 * Maps common short param names to full variable names.
 */
export function parseContractParams(url: URL): Record<string, string> {
  ...
  for (const [shortKey, fullKey] of Object.entries(mapping)) {
    const value = url.searchParams.get(shortKey);
    if (value) params[fullKey] = value;
  }

  // Always set effectiveDate to today if not provided
